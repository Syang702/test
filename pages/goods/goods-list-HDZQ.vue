...
<script>
import { getPromotionData } from '../../config/api.js'
export default {
    data() {
        return {
    ...
    onLoad(e) {
		...
		this.queryList.brandName="";
		this.queryList.orderType="";
		
		if("Y"==flag){
			this.fromIndex = true;
			this.queryList.blockName=e.blockName;
			this.queryList.brandName=e.brandName;
			this.queryList.orderType=e.orderType;
		}
    ...
    methods: {
        getGoodsList() {
			if(this.fromIndex){
				this.queryList.isFromIndexAD = 'Y';
			}else{
				this.$delete(this.queryList,'isFromIndexAD')
			}
			
            getPromotionData(this.queryList).then(res => {
				this.fillBlock(res.data.promotions.promotionList);
				this.fillBrand(res.data.promotions.brandList);
				this.fromIndex=false;
				var tempGoodsList = []
				res.data.promotions.promotionGoodsList.forEach(item => {
		...
		fillBlock(blockList){
			if(this.visitCount == '0'){
				...
				for(var i=0;i<blockList.length;i++){
					...
					if(this.fromIndex){
						if(blockInfoJson.label == this.queryList.blockName){
		...
		fillBrand(brandList){
			if(this.refreshBrand){
				...
				for(var i=0;i<brandList.length;i++){
					...
					if(brandInfoJson.label==undefined){
						brandInfoJson.label = brandList[i];
					}
					if(this.fromIndex && this.queryList.brandName!="all"){
						if(brandInfoJson.label == this.queryList.brandName){
							brandInfoJson.active = true;
							this.dropList[1].title = brandInfoJson.label;
						}
					}else{
						if(i==0){
							brandInfoJson.active = true;
