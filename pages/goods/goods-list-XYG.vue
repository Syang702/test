...
<script>
import { getCreditPurchaseGoodsList } from '../../config/api.js'
export default {
    data() {
        return {
            ...
            dropList: [
                {
                    ...
                    value: '',
                    arr: [
                        { label: '家客信用购', active: false },
                        { label: '纯线上信用购', active: false },
						{ label: 'BOSS信用购', active: false }
                    ],
                },
                {
                    title: '品牌',
    ...
    methods: {
        getGoodsList() {
            getCreditPurchaseGoodsList(this.queryList).then(res => {
                var tempGoodsList = []
				this.fillBrand(res.data.promotions.brandList)
                res.data.promotions.goodsList.forEach(item => {
					var saleControl = "";
					if(item.SALE_CONTROL != undefined && item.SALE_CONTROL.trim()!=""){
					...
					if(this.queryList.activityName!="all"){
						item.DISPLAY_NAME = this.queryList.activityName;
					}else{
						item.DISPLAY_NAME = "信用购"	;
					}
        ...
        dropTagClick(i, j) { // 下拉点击
			...
			if(i == '0'){
				this.queryList.activityName= this.dropList[i].arr[j].label.trim();
