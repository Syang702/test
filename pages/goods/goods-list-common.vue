    ...
    <view class="commen-index">
        <view class="top">
            <view class="commen-search">
                <u-search placeholder="搜索商品名称/品牌" v-model="keyword" shape="round" @custom="getGoodsList()"></u-search>
            </view>
            <view class="sort-nav">
                <view v-for="(item, index) in navList" :key="index" class="sort-nav-item" :class="[currentTag == index ? 'active' : '']"
                    ...
                    <text :style="{
                            color:
                                item.title == '仅看有货' && item.showicon == true
                                    ? '#303133'
                                    : 'transparent',
                        }" class="icon iconfont icon-benefits"></text>
                    {{ item.title }}
                    <text class="icon-sort-type">
                        <text v-if="item.title == '价格'" class="icon iconfont icon-test"
                              :class="{ 'is-active': item.priceSort, 'not-active': !item.priceSort }"></text>
                        <text v-if="item.title == '价格'" class="icon iconfont icon-test2" :class="{
                                'is-active': !item.priceSort && currentTag == index,
                                'not-active': item.priceSort,
                            }"></text>
                    </text>
...
export default {
    data() {
        return {
            ...
            navList: [
                ...
                { title: "仅看有货", showicon: false, sortType: "priceDescHasQty" },
                { title: "" },
            ],
            currentTag: 0,
    ...
    onLoad(param) {
        ...
        this.queryList.cateType2 = param.classify;
        this.queryList.sortType = "default";
        this.queryList.brandName = "all"; // 排序类型
    ...
    methods: {
        getGoodsList() {
            if (this.keyword != null && this.keyword.trim() != "") {
                this.queryList.searchKeyWord = this.keyword.trim();
            } else {
                this.$delete(this.queryList, "searchKeyWord");
            }
            ...
            getCommonGoodsList(this.queryList).then((res) => {
                ...
                res.data.goodsInfo.goodsList.forEach((item) => {
                    ...
                    if (promoteStartDataT != "" && date1 < promoteStartDataT) {
                        ...
                        item.SALE_CONTROL != undefined &&
                        item.SALE_CONTROL.trim() != ""
                    ) {
                    ...
                    if (blockNameCode != undefined) {
                        ...
                        if (retailPrice != undefined) {
                            if (retailPrice.indexOf("￥") > -1) {
                                currentPrice = retailPrice.substring(
                                    retailPrice.indexOf("￥") + 1
                                );
                            } else {
        ...
        navClick(item, index) {
            ...
            switch (item.title) {
                case "价格":
                    ...
                    this.queryList.sortType =
                        item.priceSort == false ? item.sortType1 : item.sortType2;
                    this.getGoodsList();
                    break;
                case "仅看有货":
