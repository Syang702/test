...
const getLocationDetail = (latitude, longitude) =>
    uni.$u.get(`https://apis.map.qq.com/ws/geocoder/v1/?location=${latitude.toString()},${longitude.toString()}&key=${LOCATION_API_KEY}`);
...
export const getLocationInfo = (callBack) => {
    uni.getLocation({
        type: " wgs84",
        success: async (res) => {
            let e = await getLocationDetail(res.latitude, res.longitude);
            if (e.status == 0) {
                uni.setStorage({
